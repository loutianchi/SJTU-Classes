...
# 
# Create a new instruction iaddq
# 
# The main changes are:
#	instr_valid, need_regids, need_valC
#	d_srcB, d_dstE, aluB: need a register to read, calculate and write
#	aluA: need a constant to be calculated
#	set_cc
#
# Changed the behavior for pushq followed by ret, to make it faster using jumptable.
#	f_pc: fetch the write PC immediately from E_valA
#	F_stall, D_bubble: cancel stall and bubbles, make it faster and correct.
#
# changed the behavior for rmmovq followed by mrmovq, so that there is no bubble created.
#	e_valA: add a data forwarding path from m_valM
#	F_stall, D_bubble: cancel stall and bubbles, make it faster and correct.
#	
#
#
...
word f_pc = [
	...
	## if W is ret and bubbles are inserted, normal ret
...
word e_valA = [
	M_icode in { IMRMOVQ, IPOPQ } &&
	 M_dstM in {E_srcA , E_srcB} && E_icode == IRMMOVQ: m_valM;
...
bool F_stall =
	...
	E_icode in { IMRMOVQ, IPOPQ } &&
	 E_dstM in { d_srcA, d_srcB } && D_icode != IRMMOVQ ||
	# Stalling at fetch while ret passes through pipeline
	(D_icode == IRET && E_icode != IPUSHQ) || (E_icode == IRET && M_icode != IPUSHQ) || (M_icode == IRET && W_icode != IPUSHQ)
	# To stop produce bubbles when pushq+ret encountered
	;
...
bool D_bubble =
	...
	!(E_icode in { IMRMOVQ, IPOPQ } && E_dstM in { d_srcA, d_srcB } && D_icode != IRMMOVQ ) &&
