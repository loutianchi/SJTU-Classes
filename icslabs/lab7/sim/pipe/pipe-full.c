...
long long gen_f_pc()
{
    return ((((ex_mem_curr->icode) == (I_JMP)) & !(ex_mem_curr->takebranch)
          ...
          (ex_mem_curr->icode) == (I_NOP))) ? (mem_wb_curr->valm) : ((
          (if_id_curr->icode) == (I_RET)) & ((id_ex_curr->icode) == 
          (I_PUSHQ))) ? (id_ex_curr->vala) : (pc_curr->pc));
}
...
long long gen_e_valA()
{
    return (((((ex_mem_curr->icode) == (I_MRMOVQ) || (ex_mem_curr->icode)
             == (I_POPQ)) & ((ex_mem_curr->destm) == (id_ex_curr->srca) || 
            (ex_mem_curr->destm) == (id_ex_curr->srcb))) & (
          (id_ex_curr->icode) == (I_RMMOVQ))) ? (mem_wb_next->valm) : 
      (id_ex_curr->vala));
}
...
long long gen_F_stall()
{
    return (((((((id_ex_curr->icode) == (I_MRMOVQ) || (id_ex_curr->icode)
            ...
            (I_PUSHQ)))) | (((ex_mem_curr->icode) == (I_RET)) & (
          (mem_wb_curr->icode) != (I_PUSHQ))));
}
